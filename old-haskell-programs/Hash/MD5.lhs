...
> md5_size :: Integer -> String -> String
> md5_size l s = s5
>  where s1_2 = s ++ md5_step_1_2_work (l `mod` (2^64)) ""
>        abcd = md5_step_3_init
>        abcd' = md5_step_4_main abcd s1_2
>        s5 = md5_step_5_display abcd'
...
> doit :: (XYZ -> Word32) -> ABCD -> (Rotation, Word32) -> ABCD
> doit f (a, b, c, d) (s, ki) = (d, a', b, c)
>  where mid_a = a + f(b,c,d) + ki
>        rot_a = rotL mid_a s
>        a' = b + rot_a

> md5_step_4_f :: XYZ -> Word32
> md5_step_4_f (x, y, z) = z `xor` (x .&. (y `xor` z))
> {- optimised version of: (x .&. y) .|. ((complement x) .&. z) -}

> md5_step_4_g :: XYZ -> Word32
> md5_step_4_g (x, y, z) = md5_step_4_f (z, x, y)
